...
解説記事は各媒体の公式記事API・RSSとGDELTのニュース索引から資料を探し、本文を取得できた異なる媒体が2つ以上ある場合だけ生成します。最初のテーマで資料が不足した場合は最大5テーマまで順番に試し、すべて不足した日は正常な掲載見送りとして終了します。見送ったテーマは14日間候補から外し、同じ失敗の反復を避けます。掲載・見送りの結果は `explainer_topics.json` に記録します。
